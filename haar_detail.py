...
def haar_onescale(img, integimg, haarblock_width, haarblock_height):
    #步长为1， no padding
    haarimg = np.zeros(shape=(img.shape[0] - haarblock_width + 1,
                              img.shape[1] - haarblock_height + 1),
                       dtype=np.int32)
    # plt.imshow( haarimg )
    # plt.show()
    ...
    for i in range(haarimg.shape[0]):
        for j in range(haarimg.shape[1]):
            ...
            haar_all = integimg[m][n] - integimg[m - haarblock_width][
                n] - integimg[m][n - haarblock_height] + integimg[
                    m - haarblock_width][n - haarblock_height]
            haar_black = integimg[m][n - int(haarblock_height / 2)] - integimg[
                m -
                haarblock_width][n - int(haarblock_height / 2)] - integimg[m][
                    n - haarblock_height] + integimg[m - haarblock_width][
                        n - haarblock_height]
...
def harr(haarblock_width, haarblock_height, Scale_num):
    ...
    for i in range(Scale_num):
        ...
        print('    当前 Haarblock 尺度为: ( {}, {} )'.format(
            haarblock_height, haarblock_width))
        haar_feature_onescale = haar_onescale(img, integimg, haarblock_width,
                                              haarblock_height)
